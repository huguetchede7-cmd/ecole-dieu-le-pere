...
@section('title', 'Modifier un enregistrement')

@section('page_title', 'Modifier un enregistrement')
...
<form method="POST" action="/admin/absences/{{ $absence->id }}">
    ...
    <div style="margin-bottom: 20px;">
        ...
        <input type="date" name="date" value="{{ old('date', $absence->date) }}" required
            style="width: 100%; padding: 10px 14px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px; outline: none;">
    </div>
    ...
    <div style="margin-bottom: 20px;">
        ...
        <select name="statut" required
            style="width: 100%; padding: 10px 14px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px; outline: none;">
            <option value="present" {{ old('statut', $absence->statut) === 'present' ? 'selected' : '' }}>Présent</option>
            <option value="absent" {{ old('statut', $absence->statut) === 'absent' ? 'selected' : '' }}>Absent</option>
            <option value="retard" {{ old('statut', $absence->statut) === 'retard' ? 'selected' : '' }}>Retard</option>
        </select>
    </div>

    <div style="margin-bottom: 20px;">
        <label style="display: block; font-size: 13px; font-weight: 600; color: #333; margin-bottom: 6px;">Motif (optionnel)</label>
        <input type="text" name="motif" value="{{ old('motif', $absence->motif) }}"
            style="width: 100%; padding: 10px 14px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px; outline: none;">
    </div>
