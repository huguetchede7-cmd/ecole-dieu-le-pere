...
@section('page_title', 'Modifier une inscription')
...
<form method="POST" action="/admin/inscriptions/{{ $inscription->id }}">
    @csrf
    @method('PUT')
    ...
    <div style="margin-bottom: 20px;">
        ...
        <select name="eleve_id" required
            ...
            <option value="{{ $eleve->id }}" {{ old('eleve_id', $inscription->eleve_id) == $eleve->id ? 'selected' : '' }}>
    ...
    <div style="margin-bottom: 20px;">
        ...
        <input type="text" name="annee_scolaire" value="{{ old('annee_scolaire', $inscription->annee_scolaire) }}" required
            style="width: 100%; padding: 10px 14px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px; outline: none;">
    </div>

    <div style="margin-bottom: 20px;">
        <label style="display: block; font-size: 13px; font-weight: 600; color: #333; margin-bottom: 6px;">Date d'inscription</label>
        <input type="date" name="date_inscription" value="{{ old('date_inscription', $inscription->date_inscription) }}" required
            style="width: 100%; padding: 10px 14px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px; outline: none;">
    </div>
    ...
    <div style="margin-bottom: 20px;">
        ...
        <select name="statut" required
            style="width: 100%; padding: 10px 14px; border: 1px solid #ddd; border-radius: 8px; font-size: 14px; outline: none;">
            <option value="actif" {{ old('statut', $inscription->statut) === 'actif' ? 'selected' : '' }}>Actif</option>
            <option value="inactif" {{ old('statut', $inscription->statut) === 'inactif' ? 'selected' : '' }}>Inactif</option>
        </select>
    </div>
